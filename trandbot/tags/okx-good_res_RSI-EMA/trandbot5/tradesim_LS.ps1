...
function Run-Bot {
    ...
    foreach ($symbol in $config.instruments) {
        if (CanOpenNew $symbol) {
            ...
            $longSignal  = ($rsiCurr -le $config.min_RSI) -and ($price -gt $lastEMA21) -and ($trend -eq "UP")
            $shortSignal = ($rsiCurr -ge $config.max_RSI) -and ($price -lt $lastEMA21) -and ($trend -eq "DOWN")
