...
function Run-Bot {
    foreach ($instId in $config.instruments) {
        ...
        if ($hasLong) {
            ...
            # === CLOSE LONG BY TRAILING STOP LOSS ===
        ...
        if ($hasShort) {
            ...
            if (($profit -ge ($atrDec*$tp_atr_multiplier)) -and ($trailingOrders.Count -eq 0)) {
                ...
                $trailingOrder = @{
                    ...
                    sz = ([string]$szApi)
                    callbackRatio = ([string]$callbackRatio)
                    # activePx = ([string]$currentPx) - цена
                }
        ...
        if ($hasShort) {
            ...
            # === OPEN SHORT ===
            if ($min_atr_pct -gt $atr_pct) {
                Write-Output "Low volatility"
            }
            else {
        ...
        if ($hasLong) {
            ...
            if ($min_atr_pct -gt $atr_pct) {
                ...
                if ($sellSignal -and (-not $hasShort) -and (-not $hasLong) -and ($min_atr_pct -lt $atr_pct)) {
