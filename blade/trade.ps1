...
function Place-TPSL {
    ...
    # IMPORTANT: SL stavitsya PERVYM -- count-based detekciya predpolagaet
    # condCount=1 => est SL, condCount=2 => est oba (SL + TP)
...
function Run-Bot {
    ...
    foreach ($instId in $config.instruments) {
        ...
        # ===== STATE 3: ne khvataet SL ili TP =====
        # SL stavitsya pervym: condCount=1 => est SL net TP, condCount=2 => est oba
        if ((-not $hasTP -or -not $hasSL) -and -not $hasTrailSell) {
            if (-not $hasTP -and -not $hasSL) {
                # Net nichego -- stavim oba (SL pervym)
                Place-TPSL -instId $instId -entryPx $entryPx -sz $szForOrders -info $info -config $config
            } elseif (-not $hasTP) {
                # Est SL (condCount=1), net TP -- stavim TP
                ...
                # Est TP (condCount=1 no SL pervyj ne proshel), net SL -- stavim SL
