...
function Open-Position {
    ...
    # --- Проверка ATR ---
    if ($atr -le 0 -or [double]::IsNaN($atr)) {
    ...
    try {
        ...
        LogConsole "❌ Не удалось рассчитать стоп-лосс для $symbol ($side): $($_.Exception.Message)" "ERROR"
        return
    }

    $minDist = [Math]::Max($atr * 0.1, $entryPrice * 0.0001)

    # Коррекция SL, если слишком близко или на неправильной стороне
    if ($side -eq "LONG") {
        if ($sl -ge $entryPrice -or [Math]::Abs($entryPrice - $sl) -lt $minDist) {
            $sl = [Math]::Round($entryPrice - $minDist, 8)
            LogConsole "⚠️ SL (LONG) скорректирован до $sl" "WARN"
        }
    } else {
        if ($sl -le $entryPrice -or [Math]::Abs($entryPrice - $sl) -lt $minDist) {
            $sl = [Math]::Round($entryPrice + $minDist, 8)
            LogConsole "⚠️ SL (SHORT) скорректирован до $sl" "WARN"
        }
    }

    # --- Расчет TP ---
    if ($side -eq "LONG") {
        $tp = [Math]::Round($entryPrice + [Math]::Max($atr * $tpMultiplier, $minDist), 8)
    } else {
        $tp = [Math]::Round($entryPrice - [Math]::Max($atr * $tpMultiplier, $minDist), 8)
    }

    # Дополнительная защита от отрицательного TP
    if ($tp -le 0) {
        $tp = [Math]::Round(($entryPrice + ($minDist * ($side -eq "LONG" ? 1 : -1))), 8)
        LogConsole "⚠️ TP некорректен (<=0) — выставлен запасной TP = $tp" "WARN"
    }
...
function Run-Bot {
    ...
    foreach ($symbol in $config.instruments) {
        if (CanOpenNew $symbol) {
            ...
            $shortSignal = ((($price -lt $lastEMA21) -and ($rsiCurr -le $config.min_RSI) -and ($rsi50Curr -le $config.min_RSI) -and ($trend -eq "DOWN") -and ($higherTrend_trend -eq "DOWN") -and ($higherTrend_EMA -eq "DOWN")))
